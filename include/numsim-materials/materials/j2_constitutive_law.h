...
/// (eps_p_trial, alpha_trial) provided by the integrator via Local edges.
/// The integrator calls update_source() to re-evaluate at different states.
///
/// Outputs:
...
///
/// Inputs (Local — written by integrator):
///   integrator_source::plastic_strain              — ε_p trial
///   integrator_source::equivalent_plastic_strain   — α trial
///
/// Inputs (Local — hardening re-evaluated per trial):
///   hardening_source::hardening_stress, hardening_modulus
...
public:
  ...
  void compute() {
    ...
    // Trial stress at current (eps_p, alpha) state
  ...
  /// Shear modulus accessor — needed by integrator for tangent computation.
  value_type shear_modulus() const noexcept { return m_G; }
